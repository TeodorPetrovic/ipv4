...
<template>
  <div class="space-y-6">
    <div v-if="error" class="mx-auto max-w-xl space-y-4">
      <UAlert color="error" variant="soft" :title="t('results.errors.loadFailed')"
      ...
      <div class="rounded-lg border border-default bg-elevated p-5 mx-3">
        <div class="flex items-center justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-5xl font-bold" :class="{
          ...
          <div class="text-right">
            <p class="text-2xl font-semibold text-highlighted">{{ data.attempt.score }} / {{ data.attempt.totalQuestions
              }}</p>
            ...
            <tbody>
              <tr v-for="row in data.sections.level1" :key="row.id" class="border-b border-default last:border-0">
                ...
                <td class="px-4 py-3 text-center">
                  <UInput :model-value="row.studentAnswer || ''" readonly class="w-full"
          ...
          <table class="w-full table-fixed text-sm">
            <thead class="border-b border-default bg-muted/30">
              <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold text-muted">{{ t('levels.level3.networkAddress')
                  }}
                </th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-muted">{{ t('levels.level3.correctNetwork')
                  }}
                </th>
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold text-muted">{{
                  t('levels.level3.correctBroadcast') }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in data.sections.level3" :key="row.id" class="border-b border-default last:border-0">
                ...
                <td class="px-4 py-3 text-center">
                  <UInput :model-value="row.studentNetwork || ''" readonly class="w-full"
                ...
                <td class="px-4 py-3 text-center">
                  <UInput :model-value="row.studentBroadcast || ''" readonly class="w-full"
          ...
          <table class="w-full table-fixed text-sm">
            ...
            <tbody>
              <tr v-for="row in data.sections.level5" :key="row.id" class="border-b border-default last:border-0">
                ...
                <td class="px-4 py-3 text-center">
                  <UInput :model-value="yesNoLabelI18n(row.studentAnswer)" readonly class="w-full"
                    :class="resultFieldClass(compareSubmittedValue(row.studentAnswer, row.correctAnswer, 'text'))" />
                </td>
                <td class="px-4 py-3 text-center">
          ...
          <table class="w-full table-fixed text-sm">
            <thead class="border-b border-default bg-muted/30">
              <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold text-muted">{{ t('levels.level7.networkAddress')
                  }}
                </th>
              ...
              <template v-for="row in data.sections.level7.subnets" :key="row.id">
                <tr class="border-b border-default">
                  ...
                  <td class="px-4 py-3">
                    ...
                    <UInput :model-value="row.studentMask || ''" readonly class="w-full"
